...
This document lists all API endpoints used in the Collins Frontend application. All endpoints use the centralized API configuration from `lib/api-config.ts` which reads from `NEXT_PUBLIC_API_URL` environment variable.
...
### Environment Setup:
Create a `.env.local` file in the `collins-frontend` directory:
```env
NEXT_PUBLIC_API_URL=http://localhost:5295
```
