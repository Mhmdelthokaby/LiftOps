...
namespace Collins_BackEnd.Infrastructure.Services.Maintenance
{
    public class MaintenanceService : IMaintenanceService
    {
        ...
        public async Task TransferElevatorToMaintenanceAsync(Guid elevatorId, int freeMonths = 6)
        {
            ...
            var elevator = await _unitOfWork.Repository<Collins_BackEnd.Domain.Entities.Installation.Elevator>().GetByIdAsync(elevatorId);
            if (elevator == null) 
            {
                throw new Exception($"Elevator with ID {elevatorId} not found");
            }

            var project = await _unitOfWork.Repository<Collins_BackEnd.Domain.Entities.Installation.InstallationProject>().GetByIdAsync(elevator.ProjectId);
            if (project == null) 
            {
